...
import { marksoffreedomGunma2026ImageList } from "./data";
...
export async function GET(request: Request) {
  ...
  try {
    const totalImages = marksoffreedomGunma2026ImageList.length;
    ...
    const paginatedImageFiles = marksoffreedomGunma2026ImageList.slice(
      startIndex,
      endIndex,
    );

    const paginatedImages = paginatedImageFiles.map(
      (file) => `/images/marksoffreedom/gunma-2026/${file}`,
    );
