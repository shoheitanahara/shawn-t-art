...
import { doubleslashLikeImageList } from "./data"; // 生成したデータをインポート
...
export async function GET(request: Request) {
  ...
  try {
    const totalImages = doubleslashLikeImageList.length; // 画像の総数を取得
    ...
    const paginatedImageFiles = doubleslashLikeImageList.slice(startIndex, endIndex);
    
    // paginatedImageFilesをランダムにシャッフル
    const shuffledPaginatedImages = paginatedImageFiles.sort(() => Math.random() - 0.5);
    const paginatedImages = shuffledPaginatedImages.map((file) => `/images/doubleslash-like/${file}`);
