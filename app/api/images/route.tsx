...
import { imageList } from "./data"; // 生成したデータをインポート
...
export async function GET(request: Request) {
  ...
  try {
    const totalImages = imageList.length; // 画像の総数を取得
    ...
    const paginatedImageFiles = imageList.slice(startIndex, endIndex);
    
    // paginatedImageFilesをランダムにシャッフル
    const shuffledPaginatedImages = paginatedImageFiles.sort(() => Math.random() - 0.5);
    const paginatedImages = shuffledPaginatedImages.map((file) => `/images/${file}`);
