...
import { slashcowImageList } from "./data"; // 生成したデータをインポート
...
export async function GET(request: Request) {
  ...
  try {
    const totalImages = slashcowImageList.length; // 画像の総数を取得
    ...
    const paginatedImageFiles = slashcowImageList.slice(startIndex, endIndex);
    
    const paginatedImages = paginatedImageFiles.map((file) => `/images/slashanimal/slashcow/${file}`);
