...
import { marksoffreedomIshikawa2025ImageList } from "./data";
...
export async function GET(request: Request) {
  ...
  try {
    const totalImages = marksoffreedomIshikawa2025ImageList.length; // 画像の総数を取得
    ...
    const paginatedImageFiles = marksoffreedomIshikawa2025ImageList.slice(startIndex, endIndex);
    
    // paginatedImageFilesをランダムにシャッフル
    const shuffledPaginatedImages = paginatedImageFiles.sort(() => Math.random() - 0.5);
    const paginatedImages = shuffledPaginatedImages.map((file) => `/images/marksoffreedom/ishikawa-2025/${file}`);
