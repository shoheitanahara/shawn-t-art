...
export default function Home() {
  ...
  useEffect(() => {
    const fetchImages = async (page: number) => {
      ...
      const response = await fetch(`/api/images/nft?page=${page}`); // 変更: ページ番号をクエリパラメータとして追加
    ...
    <main className="flex min-h-screen flex-col items-center justify-between px-6 md:px-24 pt-10 pb-6 md:pb-24">
      ...
      {loading ? ( // 追加: ローディング中の表示
        ...
        <div className="z-10 w-full max-w-5xl items-center justify-between font-mono text-sm lg:flex lg:grid lg:grid-cols-3">
          {images.map((image, index) => (
            <Card key={index} className="m-4 cursor-pointer overflow-hidden" onClick={() => setSelectedImage(image)}> {/* 高さを固定しない */}
              <CardContent className="grid gap-4 mt-5">
                <Image src={image} alt={`Image ${index}`} width={600} height={800} className="object-cover w-full h-auto" /> {/* 高さを自動調整 */}
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <Pagination>
        <PaginationContent className="gap-5">
          ...
          <PaginationItem>
            <PaginationNext 
              onClick={currentPage < totalPages ? () => handlePageChange(currentPage + 1) : undefined} // 修正: 最後のペーでのクリックを無効化
